...
| ------------ | -------------------------------------------------------------- |
| サイズ       | 1080 × 1920（9:16）                                            |
| 紙           | 生成り色に紙目のノイズ。上端に綴じ穴、記入欄に `#c3d6cf` の罫線 4 本 |
...
- **紙目のノイズのシードは固定する。**同じスタンプを共有し直したときに紙目が変わると、
  別の画像に見えてしまう
- **スタンプの拡大は Linear サンプリング。**保存済みのスタンプは 512x512 で、カード上は
  その倍近くになる。既定の Nearest のままだと枠の円周や斜めの線が階段状になる
- **画面を離れたら共有シートを開かない。**合成の間に離脱できるので、待っている間に
  画面が変わったら中断する（移動先の画面の上にシートが出てしまう）
- **紙の色はカード専用。**アプリの画面の背景は白のままなので `src/style/tokens.ts` には
  置かない
