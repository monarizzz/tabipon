...
- **工程ファイルは順序を知らない。**どの工程の次に自分が来るかを書かない。順序を持つのは `pipeline.ts` だけで、工程順を見直すときに触るのはそこ 1 箇所になる
...
- **中間結果は GPU テクスチャのまま渡す。**CPU コピー（`surface.ts` の `toRasterImage()`）を通すのは呼び出し側へ返す直前だけ。理由はその関数のコメントを参照
