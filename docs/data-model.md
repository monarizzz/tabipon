...
| 3. 論理設計 | テーブル・カラム・主キー・制約 | **制約以外は完了**（ER 図。カラム構成は確定済み） |
...
| 項目 | 結論 |
| --- | --- |
| 撮影時のタイムゾーンをスタンプごとに記録するか | **記録する。**文字列1カラムで、座標と同じく二度と取り直せない観測値。記録しなければ「現地時刻で表示する」への道が永久に閉じる |
| 掠れのシード列を持つか | **持つ。**v1 は中間画像を作らず元写真から毎回フル生成するため、シードが無いと同じ模様を再現できない（「v1 では中間画像を作らず、元写真を保存する」を参照） |
...
erDiagram
    STAMP {
        ...
        string captured_timezone "撮影時のタイムゾーン識別子。不変。二度と取り直せない観測値"
        string color "16進数の実値"
        string frame_id "アプリ内定数の識別子。必須"
        number scratch_seed "掠れ模様のシード。不変。同じ値なら同じ模様になる"
    }
...
**帰結として、v1 では掠れのシードを列で持つ。**掠れは再生成のたびに作り直されるため、シードが無いと
同じ模様を再現できない。現行は `seedFromStampId()` が id から導いているが、導出式を変えたり
id が変わる移行をすると既存スタンプの掠れが黙って変わるため、列として保存する。

#138 が済んで中間画像へ移行したら、この節と「保存するもの」の関係は逆転する。そのときも
...
**住所から座標に変換できなかった場合は、表示用の住所だけを更新し、座標と構造化した住所は空にする。**座標を古い値のまま残すと、住所と地図上のピンが食い違う。空にすれば食い違いは起きず、代わりにそのスタンプは地図に出なくなる。

**構造化した住所（国 / 都道府県相当 / 市区町村相当 / それ以下）も同時に空にする。**変換できなかった以上、入力された文字列を分解する手段が無いためである。古い値を残すと、画面には新しい住所が出ているのに「訪れた都道府県を数える」ような集計は古い住所を数えることになり、表示と集計が食い違う。
...
住所を更新するときは、**座標も順ジオコーディングで引き直す**（「座標と住所は双方向に更新し合う」を参照）。変換できなければ住所だけ更新し、座標は空にする。
