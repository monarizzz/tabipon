...
## エンティティは「スタンプ」と「アルバム」の2つ

スタンプとアルバムは多対多で、両者を中間テーブルでつなぐ。それ以外に1対多になる箇所は無い。
...
- 住所は1スタンプに1つ
- 色は1スタンプに1つ
- **1枚のスタンプは複数のアルバムに入り、1つのアルバムには複数のスタンプが入る**（後述）

ただし、タイムゾーンと表示言語の設定はスタンプに属さない。これらはアプリ全体の設定であり、置き場所は未確定（後述）。
...
| 画像 | スタンプ画像（PNG）のパス | 中間画像＋色＋フレームから再生成できる |
| 内容 | タイトル | 任意|
| 内容 | メモ | 任意 |
...
**リレーションはスタンプとアルバムの間の1本だけである。**`ALBUM_STAMP` は多対多を表すための中間テーブルで、現時点ではそれ自体は情報を持たない。住所・色・フレームはすべてスタンプの中に収まる。`APP_SETTING` はスタンプと関係を持たず、テーブルではなく KV で持つ方向である（後述）。図に載せているのは、何を持つかを一覧するためだけである。
...
同じ場所で何度もスタンプを押せば、同じ住所が複数の行に入る。これを「場所」エンティティにまとめて多対1にしたくなるが、しない。

- **住所はスタンプごとの値であり、共有されるモノではない。**同じ住所になるのは「たまたま同じ場所で押した」だけである
- 住所はスタンプごとに編集できる。場所を共有させると、1枚の住所を直しただけで同じ場所の他のスタンプまで書き換わってしまう
- 市区町村から都道府県が決まるので、厳密には正規化が崩れている。ただ、ここでの住所はマスタと整合を取るデータではなく、観測値・入力値をそのまま記録したものなので問題にならない

「〇〇温泉」のような場所の名前は**タイトルに入れる。**場所に名前を付けて管理し、名前を直したら紐づくスタンプ全部に反映したい、という要求が出てきたら、そのときに初めて1対多が生まれるので、この判断は見直しになる。
...
スタンプをまとめる入れ物。イメージは iPhone の写真アプリのアルバムで、1枚のスタンプを複数のアルバムに入れられる。アルバムはユーザーが作り、スタンプを入れるときはアルバムの一覧から選ぶ。

### アルバムはエンティティとして独立させる

一覧から選ぶ形なので、アルバムはスタンプとは独立して存在するモノである。スタンプごとに文字列を打つ形（タグの自由入力）にしない理由は2つ。

- **表記揺れが起きない。**「温泉」と「おんせん」が別のアルバムとして扱われることがない
- **名前を変えると、入っている全スタンプに反映される。**スタンプ側はアルバムの ID しか持たないので、名前の変更はアルバム1行の更新で済む

スタンプとアルバムは多対多になるため、中間テーブルでつなぐ。スタンプ側にアルバム ID の配列を持たせる形（1カラムに押し込む）にはしない。撮影時スナップショットと違って、アルバムは「このアルバムに入っているスタンプを引く」という絞り込みに使うのが本来の用途だからである。
...
### 属性は名前とメモ。広げる余地はテーブルが独立していることで確保する

現時点でアルバムに持たせるのは、名前とメモの2つである。

- **名前** — 必須。一覧から選ぶときの表示に使う
- **メモ** — 任意。アルバム全体についての自由記述（旅の感想など）。スタンプのメモとは別物で、1アルバムに1つしか付かないため、スタンプの住所と同じ理屈で別テーブルにせずカラムとして持つ

ただし将来の拡張には備えておきたい。その備えは、**使うかどうか分からないカラムを先に作ることではなく、アルバムを独立したテーブルにしておくこと**で満たされる。広げる先は2か所あり、どちらもカラムを足すだけで済む。

- **アルバム自体の属性**（表紙にするスタンプ、アルバム一覧での並び順など）→ アルバムのテーブルに足す
- **アルバムの中での属性**（アルバム内でのスタンプの並び順など）→ 中間テーブルに足す。同じスタンプでもアルバムごとに位置が違うので、スタンプ側には持てない

自由入力の文字列にしていたら、属性を持たせる場所そのものが無かった。

### 識別子は名前ではなく ID にする

中間テーブルがアルバムを名前で参照すると、名前を変えるたびに中間テーブルの全行を書き換えることになる。名前とは別に不変の ID を持ち、中間テーブルは ID で参照する。

### 削除の扱い

- **アルバムを削除しても、中のスタンプは消えない。**アルバムとの紐づけが消えるだけである。iPhone の写真アプリでアルバムを消しても写真が残るのと同じ
- **スタンプを削除したら、全アルバムから消える。**アルバム自体は消えない
- **空になったアルバムも残す。**アルバムはユーザーが管理している一覧なので、中身が無くなったからといって勝手に消さない

フレームと違って、アルバムは消してもスタンプの見た目は壊れない。まとめ方の情報が失われるだけで、画像にもキャッシュにも影響しない。
