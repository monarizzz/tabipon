...
`StampLocationMap` の地図はズーム・パン・回転・傾きを全て有効にする。スタンプの周りに何があるかを
確かめられるようにするため。動かした状態を元へ戻す導線は置かず、詳細を開き直せば初期位置から始まる。

地図のパンと `StampDetailMediaPager` のページ送りは同じ横方向のジェスチャで、放っておくとページャが
先に取る。`StampLocationMap` は地図カードに指が触れた / 離れたことを `onTouchStart` / `onTouchEnd`
で置く側へ流し、ページャはその間だけ `scrollEnabled` を落として地図へ譲る。
カードの外側の余白では止めないので、地図ページからでも写真ページへ戻れる。

流すのは地図があるときだけ。地図が無いカード（座標なし / `0,0`）には競合する相手がおらず、
カードがページの大半を占めるため、止めてしまうと写真ページへ戻る隙間が余白だけになる。

「終わり」は**カードの上で始まった指が全て離れたとき**に流す。`onTouchEnd` は指 1 本ごとに来るので、
ピンチ中に片方だけ離した時点で流すと、残った指で動かしたぶんをページ送りに取られる。
数えるのは `nativeEvent.identifier` で、`nativeEvent.touches` は使わない。`touches` は画面上の
全タッチなのでカードの外に置いた指まで数えてしまい、その指が離れたことはカードへ届かないため、
ページ送りが止まったままになる。

カメラの追従に `region` は使わない。`region` は制御プロップで、値が変わるたびにネイティブ側のカメラを
上書きするため、利用者が動かしたカメラを再レンダーのたびに押し戻してしまう。初回表示は
`initialRegion` に任せ、座標が変わったときだけ ref 経由の `animateToRegion()` で寄せ直す。
### 場所の編集と座標の追従
...
- **警告が指す住所は、画面にある住所と必ず一致させる。**待ち時間のあいだも場所のシートは
  入力できるので、打ち直されていたら引いた結果は捨てて打ち直した住所で引き直す。ずれたまま
  警告を出すと「このまま保存」が打ち直す前の住所を書き込むが、`unavailable` の文言には住所が
  入らないため、どちらが保存されるのか画面から判別できない
- **警告は、それを出した保存だけのもの。**待ち時間のあいだに保存を押し直せるので、同じ項目の
  保存が順番待ちに並ぶ。保存を押したとき・警告に答えたときに世代を繰り上げ、それより古い保存は
  書き込みも警告もせずに降りる。降ろさないと、答えたはずの警告が古い保存から出し直され、その
