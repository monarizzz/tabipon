...
| 判定           | 中身                                                               |
| -------------- | ------------------------------------------------------------------ |
| 残す           | 消すと再発するハマりどころ。定数の根拠、API の癖、精度・整列の制約 |
| `docs/` へ移す | 設計方針・工程順・責務の分割理由。複数ファイルにまたがる話         |
| 消す           | 作業経過、削除済みのものへの言及、コードを読めば分かること         |

- **Issue 番号だけを参照しない。**`docs/` の節か、具体的なファイルパス・関数名を指す
- **「現時点では〜」「〜する予定」を書かない。**時間が経つと嘘になる
- 同じ説明を `docs/` とコードの両方に置かない。方針は `docs/` に置き、コードからはそこを指す
...
- PR の作成・更新を行った AI は、リモート反映後、ユーザーから別途レビューを依頼されるのを待たず、同じセッションの現在の AI でローカルレビューを開始する
- ローカルレビューでは別の AI を起動せず、現在の Codex または Claude が各自のログイン済み契約をそのまま使う
