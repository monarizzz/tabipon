 ...
 * 全ストーリーのスモークテスト。
 *
 * `src/commons/` と `src/features/` の `*.stories.tsx` を実行時に走査し、Storybook の
 ...
 * ストーリーを探す範囲。
 *
 * このファイルは `src/` 直下にあるので、`__dirname` をそのまま渡すと
 * `infra/` や `utils/` まで舐めることになる。UI を持つ 2 つだけを明示する。
 */
const STORY_ROOTS = ["commons", "features"];

/** 指定ディレクトリを再帰的に辿って *.stories.tsx を集める */
...
const storyFiles = STORY_ROOTS.flatMap((root) =>
  collectStoryFiles(path.join(SRC_DIR, root)),
);
...
describe.each(
  storyFiles.map((file) => [path.relative(SRC_DIR, file), file] as const),
)("%s", (_name, file) => {
