...
export const PhotoCropArea = forwardRef<PhotoCropAreaHandle, Props>(
  function PhotoCropArea(
    ...
    if (
      ...
      fitScale =
        Math.max(
          effectiveSize / imageNaturalSize.width,
          effectiveSize / imageNaturalSize.height,
        ) * BASE_OVERSCAN;
    ...
    const panGesture = Gesture.Pan()
      ...
      .onUpdate((event) => {
        setTranslate({
          x: clamp(
            baseTranslate.current.x + event.translationX,
            -maxOffsetX,
            maxOffsetX,
          ),
          y: clamp(
            baseTranslate.current.y + event.translationY,
            -maxOffsetY,
            maxOffsetY,
          ),
        });
      ...
      () => ({
        getCroppedImageUri: async () => {
          ...
          const centerX =
            ...
            clampedTranslate.x / (scale * fitScale);
          const centerY =
            imageNaturalSize.height / 2 -
            clampedTranslate.y / (scale * fitScale);
          const originX = clamp(
            centerX - cropSize / 2,
            0,
            imageNaturalSize.width - cropSize,
          );
          const originY = clamp(
            centerY - cropSize / 2,
            0,
            imageNaturalSize.height - cropSize,
          );
